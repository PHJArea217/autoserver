...
exec /bin/sh -c 'set -eu; export ARCH=arm CROSS_COMPILE=arm-linux-gnueabihf-; cd /build_root; make olddefconfig; \
make -j 5; make INSTALL_MOD_STRIP=1 INSTALL_MOD_PATH=/build_root/kernel-output modules_install; \
...
mksquashfs /build_root/kernel-output/usr /build_root/kernel-output/lib/modules /build_root/kernel-output/k_mod.img -comp xz -b 1048576 -Xdict-size 100% -Xbcj x86,arm,armthumb -noappend -all-root'
EOF
