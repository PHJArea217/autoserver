...
docker run --rm -v /_autoserver/_ctr-script-build-output_3/guacamole:/build_out --entrypoint= -u root guacamole/guacamole /bin/sh -c 'mkdir -p /etc/guacamole && tar c /bin /etc /lib /lib64 /opt /sbin /usr /var > /build_out/rootfs.tar'
docker run --rm -v /_autoserver/_ctr-script-build-output_3/guacamole-guacd:/build_out --entrypoint= -u root guacamole/guacd /bin/sh -c 'tar c /bin /etc /lib /lib64 /sbin /usr /var > /build_out/rootfs.tar'
