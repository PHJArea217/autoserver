...
RUN dpkg-divert --add --no-rename --divert /usr/bin/qemu-arm-static.arm /usr/bin/qemu-arm-static
...
ipxe-qemu isc-dhcp-client isc-dhcp-server iso-codes iw kbd keyboard-configuration klibc-utils kmod kpartx \
less libarchive-tools libcap2-bin libnss-mdns libnss-myhostname libsox-fmt-all linux-base lm-sensors login lsof \
ltrace man-db manpages manpages-dev mc mdadm mime-support minicom minidlna mlocate moreutils mplayer mtools \
...
rtkit runc screen seabios sharutils socat sox squashfs-tools ssh-import-id strace sudo systemd tmux \
...
RUN env DEBIAN_FRONTEND=noninteractive apt-get -y install --no-install-recommends \
